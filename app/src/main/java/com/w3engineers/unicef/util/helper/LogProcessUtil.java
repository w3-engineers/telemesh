...
import com.w3engineers.unicef.telemesh.ui.meshlog.MeshLogModel;
    ...
    private void saveLogInSd(String log) {
        ...
        try {
            ...
            if (crashFile.exists()) {
                ...
                String logData = getLogDateWise(aBuffer, log);

                sendCurrentLog(log, logData.contains(Constants.AppConstant.DASHES));

                aBuffer = aBuffer + (logData.contains(Constants.AppConstant.DASHES) ? "\n" : "") + logData;
                bufferedReader.close();
